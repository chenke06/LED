...
{
    .light_control_auto_flag = 0,
    .test_flag = 0,
	  .led_on_flag = 0,
	  .led_off_flag = 0,
	  .light_test_power_flag = 0,
	  .light_power = 0,
    .patrol_time_flag = 0,
    .patrol_time_value = 0,
    .timer_mode_flag = 0,
	
	  .work_time_tonight_flag = 0,       // min
...
//             s_crc_len = 0;
         ...
         case 9:
             ...
             break;            					 
				 case 10:
             BYTE1(g_lora_protocol_rec.w_n_reg_data_w[g_lora_protocol_rec.w_n_reg_i_w]) = rec;
				     s_crc_buf[s_crc_len] = rec;
             s_crc_len++;
				     s_rec_step = 11;
				     break;
				 case 11:
					   BYTE0(g_lora_protocol_rec.w_n_reg_data_w[g_lora_protocol_rec.w_n_reg_i_w]) = rec;
	           s_crc_buf[s_crc_len] = rec;
             s_crc_len++;
				     if (g_lora_protocol_rec.w_n_reg_i_w >= g_lora_protocol_rec.w_word_len_b - 1){
                 s_rec_step = 100;
...
uint8_t lora_analysis_thread()
{
    ...
    uint16_t reg_region = 0;
	  __IO uint16_t *reg_region_r_sp;
	 	__IO uint16_t *reg_region_w_sp;
    ...
    if (g_lora_protocol_rec.stop_b == 1){
        ...
        if (crc_check == g_lora_protocol_rec.check_w){    //一段数据接收完成
            ...
            s_send_lora_buf[2] = BYTE1(g_lora_protocol_rec.reg_w);
            s_send_lora_buf[3] = BYTE0(g_lora_protocol_rec.reg_w);
				    reg_region = g_lora_protocol_rec.reg_w & 0XFF00;
					  if (reg_region == 0X4000){
                ...
                g_lora_protocol_rec.reg_array_addr = g_lora_protocol_rec.reg_w - 0XE080;
					  ...
					  if (g_lora_protocol_rec.function_b == 0X03){
						    s_send_lora_buf[4] = (uint8_t) (g_lora_protocol_rec.r_word_len_w * 2);
							  s_send_lora_len = 5;
							  for (uint8_t i = 0; i < g_lora_protocol_rec.r_word_len_w ; ){
									  s_send_lora_buf[5 + i] = BYTE1(reg_region_r_sp[g_lora_protocol_rec.reg_array_addr + i]);
                    s_send_lora_buf[6 + i] = BYTE0(reg_region_r_sp[g_lora_protocol_rec.reg_array_addr + i]);
									  s_send_lora_len = s_send_lora_len + 2;
									  i = i + 2;
								}
						}
						else if (g_lora_protocol_rec.function_b == 0X06){
						    s_send_lora_buf[4] = BYTE1(g_lora_protocol_rec.w_sigle_reg_data_w);
						    s_send_lora_buf[5] = BYTE0(g_lora_protocol_rec.w_sigle_reg_data_w);
					      s_send_lora_len = 6;
                ...
                g_lora_protocol_rec.w_reg_updata = 1;
						...
						else if (g_lora_protocol_rec.function_b == 0X10){
						    s_send_lora_buf[4] = BYTE1(g_lora_protocol_rec.w_byte_len_w);
						    s_send_lora_buf[5] = BYTE0(g_lora_protocol_rec.w_byte_len_w);
					      s_send_lora_len = 6; 
							  
							  for (uint8_t i = 0; i < g_lora_protocol_rec.w_word_len_b ; i++){
									  g_lora_protocol_rec.reg_array_addr = g_lora_protocol_rec.reg_array_addr + i;
                    reg_region_w_sp[g_lora_protocol_rec.reg_array_addr] = g_lora_protocol_rec.w_n_reg_data_w[i];						  
								}
                g_lora_protocol_rec.w_reg_updata = 1;							
						}
						else{
				    ...
				    s_send_lora_buf[s_send_lora_len++] = BYTE0(send_lora_crc);
		        s_send_lora_buf[s_send_lora_len++] = BYTE1(send_lora_crc);
						delay_ms(1);
...
void send_lora_unusual(uint16_t unusual_b)
{
    ...
    str_abnormal[2] = BYTE1(unusual_b);
    str_abnormal[3] = BYTE0(unusual_b);
    unusual_check = crc16(str_abnormal, 4);
    str_abnormal[4] = BYTE0(unusual_check);
    str_abnormal[5] = BYTE1(unusual_check);
...
//PD0
    ...
    updata_time_data(&updata_time_r); 
	  ...
	  updata_rn8209_data(&rn8209_data);

    float temp;
    int16_t temp_int;
    //updata_ds18b20_data(&temp);
	  updata_ntc_value(&temp);
    temp_int =(int16_t) temp;

	  uint16_t light_percent;
    light_percent = g_load_mode.light_power;
	
    uint16_t light_time_tonight;
		light_time_tonight = g_load_mode.work_time_tonight ;
		
		uint32_t light_time_total;
		light_time_total = g_load_mode.work_time_total;
		
		uint16_t use_energy_tonight;
	  uint16_t use_energy_total;
		
		uint16_t led_lux_value;
		float led_lux_cs;
		//updata_opt3001_data(&led_lux_value);
		updata_light_value(&led_lux_cs);
		led_lux_value = (int16_t) led_lux_cs;
...
//		printf("temp%d\r\n",temp_int);
    ...
    g_pd0_r_reg[16] = WORD1(light_time_total);
		g_pd0_r_reg[17] = WORD0(light_time_total);
		
		g_pd0_r_reg[18] = use_energy_tonight;
	  g_pd0_r_reg[19] = WORD1(use_energy_total);
    g_pd0_r_reg[20] = WORD0(use_energy_total);
    ...
    if (g_lora_protocol_rec.w_reg_updata == 1){
		    uint16_t reg_temp = 0;
			  ...
			  if (reg_temp == 0X4000){
			      s_updata_time_w.tm_year = BYTE1(g_pd0_w_reg[0]);
            s_updata_time_w.tm_mon = BYTE0(g_pd0_w_reg[0]);
	
            s_updata_time_w.tm_mday = BYTE1(g_pd0_w_reg[1]);
            s_updata_time_w.tm_hour = BYTE0(g_pd0_w_reg[1]);
	
            s_updata_time_w.tm_min = BYTE1(g_pd0_w_reg[2]);
            s_updata_time_w.tm_sec = BYTE0(g_pd0_w_reg[2]);

            s_updata_time_w.tm_wday = BYTE1(g_pd0_w_reg[3]);
				}
				if (reg_temp == 0XDF00){
...
void lora_send_timer()
{
     ...
     send_timer_str[1] = s_lora_send_timer.function_b;
	   send_timer_str[2] = BYTE1(s_lora_send_timer.reg_w);
     send_timer_str[3] = BYTE0(s_lora_send_timer.reg_w);
	   send_timer_str[4] = s_lora_send_timer.data_len_b;
	
     len_str = 5;
	   uint8_t j = 0;
     for (uint8_t i = 0; i < s_lora_send_timer.data_len_b;i++){
         send_timer_str[5 + j] = BYTE1(s_lora_send_timer.reg_data_w[i]);
         send_timer_str[6 + j] = BYTE0(s_lora_send_timer.reg_data_w[i]);
			   j = j + 2;
         len_str = len_str +2;
		 }
		 s_lora_send_timer.check_w  = crc16(send_timer_str, len_str);
		 send_timer_str[len_str] = BYTE0(s_lora_send_timer.check_w);
     send_timer_str[len_str + 1] = BYTE1(s_lora_send_timer.check_w);
     len_str = len_str +2;
