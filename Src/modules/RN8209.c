 ...
 * 描述  ：清校表参数函数，清rn8209寄存器
         vu = 220 / 1621 = 0.13571869   电压通道输入值   额定电压
         vi = 5 * 0.1  *16                 额定电流输入时 电流通道的电压(引脚电压 * 放大倍数)
         Un = 220                       额定电压值
         Ib = 5                         额定电流值
         EC = 51200                      电表常数，imp/kwh ，多少脉冲一度电
         HFConst =  INT[23.2075 * vu * vi] * 10^11 * / (EC * Un * Ib)] = 44739   = AEC3                 
...
void rn8209_init(void)
{
    ...
    s_RN8209_firm_para.HFConst = 0xAEC3;      //0x1000;
    s_RN8209_firm_para.PStart = 0x0060;
    s_RN8209_firm_para.QStart = 0x0120;
    s_RN8209_firm_para.GPQA = (9.0728 * 32768);//0x0000;
    ...
    s_RN8209_firm_para.EMUCON2 = 0x300;
	  s_RN8209_firm_para.KUrms=0.000185271739f;                                        //电压校准系数
	  s_RN8209_firm_para.KIArms= 0.001325249137f;    		//电流校准系数MA/10
	//s_RN8209_firm_para.KIBrms= 0x00000000;
	
    s_RN8209_firm_para.KPrms= 3.22155 * (10^12)/(2^32 * s_RN8209_firm_para.HFConst * 51200);
...
{ 
    ...
    //计算功率
//    s_RN8209_front.PubData.Pw = s_RN8209_front_tmp.Pw[0] * s_RN8209_firm_para.KPrms * 13.75f - 15;

		s_RN8209_front.PubData.Pw = s_RN8209_front_tmp.Pw[0] * s_RN8209_firm_para.KPrms * 11.875;
		
    if (PFlag){
        s_RN8209_front_PubData_Pw_flag = 1;
		}
    //电流电压计算
    s_RN8209_front.PubData.U = (0x7fffff & (s_RN8209_front_tmp.UI[2])) * s_RN8209_firm_para.KUrms;
    TempI = s_RN8209_front_tmp.UI[0] * s_RN8209_firm_para.KIArms;   
   ...
   // s_RN8209_front.PubData.Pf = (s_RN8209_front.PubData.Pw * 1000) / (s_RN8209_front.PubData.U * s_RN8209_front.PubData.Ia / 10000); //((u16)((float)TempU / 8388.608));
 ...
 * 函数名：updata_rn8209_data
	u16			U;			    		//电压NNN.N (V)  2240
	u32	    Ia;			    		//电流N.N(A)	   468	
	u32	    In;         		//---零线电流    
	int16_t	Pw;			        //---瞬时有功p   10100
	u16			Pf;			    		//---功率因数    963
	u16			Angle;		  		//---相角NNN.N	
...
{
    rn8209_data->PubData.U = s_RN8209_front.PubData.U * 10;
    rn8209_data->PubData.Ia = s_RN8209_front.PubData.Ia / 10;
    rn8209_data->PubData.Pw = s_RN8209_front.PubData.Pw * 100;
    rn8209_data->PubData.Pf = rn8209_data->PubData.Pw * 100000/ (rn8209_data->PubData.U * rn8209_data->PubData.Ia); //s_RN8209_front.PubData.Pf;
    rn8209_data->PubData.Frequency = s_RN8209_front.PubData.Frequency;
	  rn8209_data->PubData.Angle    = s_RN8209_front.PubData.Angle;
}
